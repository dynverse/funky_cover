...
def renderToFolder(renderFolder='rendering', renderName='render', resX=800, resY=800, resPercentage=100, animation=False, frame_end=None):
    ...
    if True:#bpy.context.space_data is None:
...
def lighten_color(color, amount=0.5):
    ...
    return colorsys.hsv_to_rgb(c[0], c[1], amount)


def middle_point(obj):
    bpy.context.scene.update()
...
def look_at(obj_camera, obj = None, point = None):
    obj_camera.constraints.clear()

    if obj is not None:
        point = middle_point(obj)
...
def delete_recursive(parent, scene = bpy.context.scene):
    for child in parent.children:
        if len(child.children) > 0:
            delete_recursive(child)
    objects.remove(parent, do_unlink=True)
    

def hide(parent, to_hide = True, frame = None, scene = bpy.context.scene):
    for child in parent.children:
        hide(child, to_hide = to_hide, frame = frame, scene = scene)
    parent.hide = to_hide
    if frame is not None:
        parent.keyframe_insert("hide", frame = frame)